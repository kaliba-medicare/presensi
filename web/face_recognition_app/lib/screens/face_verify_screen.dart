...
class _FaceVerifyScreenState extends State<FaceVerifyScreen> {
  ...
  Future<void> _captureAndVerifyFace() async {
    ...
    try {
      ...
      // TODO: Send embedding to your Laravel API for verification
      // final response = await ApiService().verifyFace(embedding);
      
      // Simulate verification result
      final isVerified = true;
      _similarity = 0.92; // This would come from the API response
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              isVerified 
                ? 'Face verified successfully! (Similarity: ${(_similarity! * 100).toStringAsFixed(1)}%)' 
                : 'Face verification failed'
            ),
          ),
        );
      ...
      setState(() {
        _status = 'Failed to verify face: ${e.toString()}';
      });
      
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to verify face: ${e.toString()}')),
        );
      }
    } finally {
      setState(() {
